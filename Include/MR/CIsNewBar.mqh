...
class CIsNewBar {
 ...
 public:
   ...
   uint              GetRetCode() const      {
      return(m_retcode);       // Result code of detecting new bar
   }
   datetime          GetLastBarTime() const  {
      return(m_lastbar_time);  // Time of opening new bar
   }
   int               GetNewBars() const      {
      return(m_new_bars);      // Number of new bars
   }
   string            GetComment() const      {
      return(m_comment);       // Comment of execution
   }
   string            GetSymbol() const       {
      return(m_symbol);        // Symbol name
   }
   ENUM_TIMEFRAMES   GetPeriod() const       {
      return(m_period);        // Chart period
   }
   //--- Methods of initializing protected data:
   void              SetLastBarTime(datetime lastbar_time) {
      m_lastbar_time=lastbar_time;
   }
