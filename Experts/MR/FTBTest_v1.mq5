...
void ftb() {
   ...
   if (rates[ArraySize(rates)-1].time == D'2022.04.28 52:00:00') {
      bool stop = true;
   }
   ...
   for(int i=1; i<ArraySize(snd); i++) {
      if (snd[i-1].type == snd[i].type) {
         MqlRates tempRates[];
         CopyRates(_Symbol, PERIOD_CURRENT, snd[i-1].rates.time, snd[i].rates.time, tempRates);
         if (ArraySize(tempRates) > 2 && ArraySize(tempRates) <= 5) {
            if (snd[i].type == "demand") {
               if (snd[i].rates.low > snd[i-1].rates.low+(snd[i-1].rates.high-snd[i-1].rates.low)*0.382) {
            ...
            else if (snd[i].type == "supply") {
               if (snd[i].rates.high < snd[i-1].rates.high-(snd[i-1].rates.high-snd[i-1].rates.low)*0.382) {
                  bool hasretrace = false;
                  for (int j=1; j<ArraySize(tempRates)-1; j++) {
                     if (tempRates[j].open < tempRates[j].close) {
                        hasretrace = true;
...
void findSupplies(MqlRates &rates[], int inpPeriod, RatesPoint &snd[]) {
