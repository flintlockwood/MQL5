...
bool checkAllCSPattern(ENUM_TIMEFRAMES tf) {
   bool redraw = false;
   ...
   bool mFlag = checkMarubozu(rates[period-1], dvp, mSignal);
   if (mFlag) {
   ...
   bool rFlag = checkRejection(rates[period-1], rangeTh, rSignal);
   if (rFlag) {
   ...
   bool hFlag = checkHarami(rates2, dvp, hSignal);
   if (hFlag) {
   ...
   bool sFlag = checkStar(rates3, dvp, sSignal);
   if (sFlag) {
   ...
   bool tFlag = checkTripleCS(rates3, dvp, tSignal);
   if (tFlag) {
   ...
   bool tiFlag = checkThreeInside(rates3, dvp, tiSignal);
   if (tiFlag) {
...
bool checkMarubozu(MqlRates &rates, DvpRates &dvp, SignalBase &signal)
{
   ...
   if (bodylength == rangelength) {
      if (greencandle) {
         if ((rates.open < dvp.Val && rates.close > dvp.Val) ||
             (rates.open < dvp.Poc && rates.close > dvp.Poc) ||
             (rates.open < dvp.Vah && rates.close > dvp.Vah)) {
            MarubozuSignal s;
            s.price = rates.low;
            s.time = rates.time;
...
bool checkRejection2(MqlRates &rates, double th, SignalBase &signal)
{
   bool ret = false;
...
bool checkEnGulfing(MqlRates &rates[], DvpRates &dvp, SignalBase &signal)
{
   ...
   if (b1 < 0 && b2 > 0 && ratio >= 0.8) {
      if (MathAbs(b2) > 2*MathAbs(b1)) {
         if ((rates[1].open < dvp.Val && rates[1].close > dvp.Val) ||
             (rates[1].open < dvp.Poc && rates[1].close > dvp.Poc) ||
             (rates[1].open < dvp.Vah && rates[1].close > dvp.Vah)) {
            EngulfingSignal s;
            s.price = rates[1].low;
            s.time = rates[1].time;
            s.type = SIGNAL_TYPE_BULLISH;
            ...
            else if (rates[1].open < dvp.Poc && rates[1].close > dvp.Poc) {
               s.KeyLevel = dvp.Poc;
            }
            else if(rates[1].open < dvp.Vah && rates[1].close > dvp.Vah) {
               s.KeyLevel = dvp.Vah;
            }
            signal = s;
            ret = true;
...
bool checkHarami(MqlRates &rates[], DvpRates &dvp, SignalBase &signal)
{
   if (ArraySize(rates) != 2) {
      return false;
   }
   ...
   if (bodyMax0 > bodyMax1 && bodyMin0 < bodyMin1 && ratio0 >= 0.8 && ratio1 <= 0.2) {
      if (MathAbs(rates[1].close - dvp.Val) <= 10*_Point ||
          MathAbs(rates[1].close - dvp.Poc) <= 10*_Point ||
          MathAbs(rates[1].close - dvp.Vah) <= 10*_Point) {
         // Bullish Harami
...
bool checkStar(MqlRates &rates[], DvpRates &dvp, SignalBase &signal)
{
...
bool checkTripleCS(MqlRates &rates[], DvpRates &dvp, SignalBase &signal)
{
   if (ArraySize(rates) != 3) {
      return false;
   }
   ...
   if (ratio0 > 0.4 && ratio1 > 0.4 && ratio2 > 0.4) {
      ...
      if (bodyWhite0 && bodyWhite1 && bodyWhite2) {
         if (rates[0].open < dvp.Val && rates[2].close > dvp.Val) {
            TripleCsSignal s;
            s.price = rates[2].low;
            s.time = rates[2].time;
            s.type = SIGNAL_TYPE_BULLISH;
      ...
      // check bearish three black soldier
         ...
         if (rates[0].open > dvp.Vah && rates[2].close < dvp.Vah) {
            TripleCsSignal s;
            s.price = rates[2].high;
            s.time = rates[2].time;
            s.type = SIGNAL_TYPE_BEARISH;
...
bool checkThreeInside(MqlRates &rates[], DvpRates &dvp, SignalBase &signal)
{
   if (ArraySize(rates) != 3) {
      return false;
   }
   ...
   if (rates[1].high <= rates[0].high && rates[1].low >= rates[0].low) {
      ...
      if (!bodyWhite0 && bodyWhite1 && bodyWhite2 && rates[1].close > rates[0].close && rates[2].close > rates[0].open) {
         if (rates[1].open < dvp.Val && rates[2].close > dvp.Val) {
            ThreeInsideSignal s;
            s.price = rates[2].low;
            s.time = rates[2].time;
            s.type = SIGNAL_TYPE_BULLISH;
      ...
      // check three inside down
         ...
         if (rates[1].open > dvp.Vah && rates[2].close < dvp.Vah) {
            ThreeInsideSignal s;
            s.price = rates[2].high;
            s.time = rates[2].time;
            s.type = SIGNAL_TYPE_BEARISH;
